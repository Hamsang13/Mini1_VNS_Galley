			...
			<div id="wrapper">
				...
				<%Object obj = request.getAttribute("product");
					...
					<div id="main">
						<div class="inner">
							<h1><%=vo.getP_title() %></h1>
							<a href="Controller?type=shop&searchtype=0&search=<%=vo.getP_artist()%>" style="padding-bottom: 10%; overflow: hidden; text-decoration: none; border-bottom: 0px; "><h3><%=vo.getP_artist() %></h3></a>
							...
							<h2><%=vo.getP_price() %> &#8361;</h2>
							<p><button value="구매" onclick="location.href='Controller?type=buy&p_idx=<%=pvo.getP_idx()%>&m_idx=<%=mvo.getM_idx()%>'">구매</button></p>
							</div>
							</div>
							<br/><br/><br/>
						</div>
